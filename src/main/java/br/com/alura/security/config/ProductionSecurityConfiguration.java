...
public class ProductionSecurityConfiguration extends WebSecurityConfigurerAdapter {
	...
	@Bean // Spring entende que tem q devolver o authentication manager e aí é possivel
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests().antMatchers(HttpMethod.GET, "/topicos").permitAll()
				...
				.antMatchers(HttpMethod.DELETE, "/topicos/*").hasRole("MODERADOR").anyRequest().authenticated().and()
				.csrf().disable().sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS) // Avisa
																												// pro
																												// spring
																												// que a
																												// autenticaçao
				...
				.and().addFilterBefore(new AutenticacaoViaTokenFilter(tokenService, UsuarioRepository),
						UsernamePasswordAuthenticationFilter.class); // avisa ao spring qual filtro será iniciado antes
																		// pois nao dá pra chamar via anotação
	}
